	...
	<div class="ps-page ps-page--activity">
		...
		<div class="ps-activity">
			...
			<?php } elseif($post->post_type == 'peepso-post') { ?>
				...
				<input type="hidden" id="peepso_post_id" value="<?php global $post; echo esc_attr($post->ID); ?>" />
			...
			<div class="ps-activity__container">
				<div id="ps-activitystream-recent" class="ps-posts <?php echo esc_attr($small_thumbnail) ? '' : 'ps-posts--narrow' ?>" style="display:none"></div>
				<div id="ps-activitystream" class="ps-posts <?php echo esc_attr($small_thumbnail) ? '' : 'ps-posts--narrow' ?>" style="display:none"></div>
				<?php
						...
						if (is_object($activity) && $activity->act_access == PeepSo::ACCESS_PUBLIC)
						{
							...
							<noscript>
								<article class="peepso_post">
									<h2><?php
										echo sprintf(
											__('Post by %s on %s', 'matebook'),
											'<a href="' . esc_url($user->get_profileurl()) . '">' . trim(strip_tags($user->get_fullname())) . '</a>',
				...
				<div id="ps-no-posts" class="ps-posts__empty"><?php echo esc_html__('No posts found.', 'matebook'); ?></div>
				<div id="ps-no-posts-match" class="ps-posts__empty"><?php echo esc_html__('No posts found.', 'matebook'); ?></div>
				<div id="ps-no-more-posts" class="ps-posts__empty"><?php echo esc_html__('Nothing more to show.', 'matebook'); ?></div>
